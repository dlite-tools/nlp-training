...
class AGNewsDataModule(pl.LightningDataModule):
    ...
    def generate_batch(self, batch: Any, train: bool = False) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        for text in texts:
            ...
            docs.append(torch.tensor(doc.output))

        docs = torch.cat(docs)  # xtorch.nn.utils.rnn.pad_sequence(docs, batch_first=True, padding_value=1)
        offsets = torch.tensor(offsets, dtype=torch.long)[:-1].cumsum(dim=0)
        return torch.tensor(labels), docs, offsets
